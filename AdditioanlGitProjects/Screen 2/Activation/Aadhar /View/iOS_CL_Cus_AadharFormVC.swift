...
class iOS_CL_Cus_AadharFormVC: AppViewController{
    ...
    var timmer = Timer()
    var count = 45
    var delegate: AadharForm_Delegate?
	var profileName: String{
		self.delegate?.profileName ?? ""
	}
	var validateBtnStatus: AadharFormValidateBtnEnum = .validate
	var proceedBtnStatus: AadharFormProceedBtnEnum = .proceed
	
	var viewmodels: AadharFormVM_InputProtocol
	init(customerDetails: ObjCustomerBasicDetailsActivation,viewmodels: AadharFormVM_InputProtocol){
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		self.viewmodels.aadharCountCheck() { count in
			self.viewmodels.getAadharDetailsAPI(countStatus: count)
		}
	}
	...
	@IBAction func didSelectEditAddress(_ sender: UIButtonDesignable) {
		self.viewmodels.aadharCountCheck { status in
			if status {
...
extension iOS_CL_Cus_AadharFormVC: AadharFormVM_OutputProtocol{
...
//	profile Name Updated
	...
	func aadharDetailsConfig(aadharNumber: String,aadharName: String, editStatus: Bool) {
		...
		self.editAadharBtn.isHidden = !editStatus
		self.limitExceedView.isHidden = editStatus
		self.aadharInfoView.isHidden = true
		self.aadharNumberTF.text = aadharNumber
		...
		self.validateBtn.isHidden = true
		self.otpContainerView.isHidden = true
		self.aadharStatusView.isHidden = false		// need to confirm
...
extension iOS_CL_Cus_AadharFormVC{
...
extension iOS_CL_Cus_AadharFormVC: SuccessSubmissionProtocol{
...
extension iOS_CL_Cus_AadharFormVC: UITextFieldDelegate{
...
enum AadharFormValidateBtnEnum{
	case validate
	case otpSubmit
}


enum AadharFormProceedBtnEnum{
	case next
	case proceed
	case aadharSubmit
}
