//
//  iOS_CL_Cus_PersonalDetailsVC.swift
//  ClubLink_iOS
//
...
class iOS_CL_Cus_PersonalDetailsVC: AppViewController {
	...
	@IBOutlet weak var spouceDOBTitleLbl: UITextField!
	...
	var delegate: PersonalDetails_Delegate?
	var personal =  PersonalDetailsVCModels()
	let Requiredformatter = DateFormatter()
	var superStarData: HierarchyMapDetails?
	
	var viewmodels: PersonalDetailsVM_InputDelegate
	init(customerDetails: ObjCustomerBasicDetailsActivation,viewmodels: PersonalDetailsVM_InputDelegate){
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		self.viewmodels.getPersonalDetails(userName: self.customerDetails.loyalityID ?? "")
	}
	...
	@IBAction func didSelectCityName(_ sender: UIButton) {
		...
		guard self.personal.stateID != -1 && self.personal.stateID != 0 else{
	...
	@IBAction func didSelectSubmitBtn(_ sender: GradientButton) {
		...
		}else if !(self.delegate?.imageUploadStatus ?? false){
			ToastMessage.shared.showToast(message: "Please Upload Profile Image")
		}else{
			//api call
...
extension iOS_CL_Cus_PersonalDetailsVC: PersonalDetailsVM_OutputDelegate,SuccessSubmissionProtocol{
...
//			self.personalDetailsDataConfig(data: personalDetails)
	...
	private func personalDetailsDataConfig(data: ObjCustomer){
		...
		if let address = data.address1,address != "--"{
			self.firmAddressTF.text = address
		}
		
		if let address3 = data.address3,address3 != "--"{
			self.permanentAddressTF.text = address3
		}
		
		if let address2 = data.address2,address2 != "--"{
			self.temporaryAddressTF.text = address2
		}
		...
		if let customerMobile = data.customerMobile,customerMobile != "--"{
			self.whatsappNumberTF.text = customerMobile
		}
		
		if let customerEmail = data.customerEmail,customerEmail != "--"{
			self.emailTF.text = customerEmail
		}
		
		if let customerZip = data.customerZip,customerZip != "--"{
			self.pinCodeTF.text = customerZip
		}
		
		if let dob = data.jdob,dob != "1/1/1900 00:00:00" && dob != ""{
		...
		if data.maritalStatus == 1,let anniversary = data.anniversary,anniversary != "0001-01-01T00:00:00"{
		...
		if data.whatsAppNumber ?? "" == "" || data.whatsAppNumber ?? "" == "--"{
			...
			self.submitBtn.setTitle("Proceed", for: .normal)
		}else{
			self.personal.isFormEditable = false
			self.submitBtn.setTitle("Next", for: .normal)
		}
		self.personalDetailsIsEditable(status: self.personal.isFormEditable)
	...
	private func personalDetailsIsEditable(status : Bool){
		...
		self.temporaryAddressTF.isEnabled = status
		self.mobileNumberTF.isEnabled = status
		self.whatsappNumberTF.isEnabled = status
		self.emailTF.isEnabled = status
		self.pinCodeTF.isEnabled = status
		self.spouceNameTF.isEnabled = status
		self.child1TF.isEnabled = status
		self.child2NameTF.isEnabled = status
		self.child3NameTF.isEnabled = status
	}
...
extension iOS_CL_Cus_PersonalDetailsVC: UITextFieldDelegate{
...
extension iOS_CL_Cus_PersonalDetailsVC: SearchableDropDownDelegate{
...
extension iOS_CL_Cus_PersonalDetailsVC: DatePickerDelegate{
...
extension iOS_CL_Cus_PersonalDetailsVC{
	...
	private func personalDetailsSavingApi(){
		...
		let request : [String : Any] = [
			"ActionType": 9,
			"ActorId": customerDetails.user_ID ?? "",
...
extension iOS_CL_Cus_PersonalDetailsVC: RegisterOTPVC_Delegate{
